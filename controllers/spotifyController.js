...
authRouter.get("/login", auth(["clerk"]), (req, res) => {
  ...
  res.json({redirectUrl: `https://accounts.spotify.com/authorize?response_type=code&client_id=${CLIENT_ID}&scope=${scope}&redirect_uri=${REDIRECT_URI}`});
});
...
async function checkSubscription(res) {
  try {
    ...
    if (error.response) {
      if(error.response.data.error.reason === "PREMIUM_REQUIRED") res.status(403).json({ error: "Premium required" });
